...
program makesAC
    ...
    use mLibraryFunctions,              only : H, D, F
    ...
    character ( len =   5 ) :: och5 = ''
        ...
        ! convert to long integer for integration loop
        proc = myproc
        numproc = mynumproc
        ...
        do ijk = proc + 1, Npoints, numproc
          ...
          *exp(-at*as**3*H(phi,gphi,pi, as, m, Mass ))*D(phi, gphi, pi, as, m, Mass )
          C = - sixth*pistep*cos(as**3*F(phi, gphi, dphi, pi, as, m, Mass )) &
          *exp(-at*as**3*H(phi,gphi,pi, as, m, Mass ))*H(phi,gphi,pi, as, m, Mass ) &
     ...
     pido: do l = 1, thisNpi
        ...
        if ( pistep * exp ( -at * as ** 3 * H ( phi, gphi, pi, as, m, Mass ) ) * D( phi, gphi, pi, as, m, Mass ) < eps ) exit
