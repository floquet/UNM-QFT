program ammonte
  ...
  real(8),parameter:: two = 2.0_8, three = 3.0_8
  ...
  do k = 1, Ndphi+1
     do j = 1, Ngphi+1
        do i = 1, Nphi
           ...
           phi(i) = real(i-1,8)*phistep
           gphi(j) = real(j-1,8)*gphistep
           dphi(k) = real(k-1,8)*dphistep
        end do
     end do
  end do
  close(4)
  ...
  write(8,'(3(f12.8,2x))')  maxPhi, maxGphi, maxDphi
